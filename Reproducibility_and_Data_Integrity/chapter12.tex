...
\title{Chapter 12: FAIR and Reproducible Reporting of Molecular Docking Studies}
\author{Elvis Martis}
\date{}
\maketitle
...
Concerns about the reproducibility of findings in computational science have attracted increasing attention in recent years. In many instances, reliance on proprietary software or datasets not available in the public domain limits independent researchers' ability to critically evaluate the reliability of the underlying methodologies and, consequently, the validity of the reported results. The recent criticism surrounding the publication of the AlphaFold3\cite{CH12_AF3} study in Nature has further reinforced the need for policies that promote transparency and open access to both data and computational methods. Such practices would enable independent validation of findings and expand the scientific impact of the work beyond the scope originally envisioned by the authors\cite{ CH12_plos2025tenDocking}. Despite the widespread use of molecular docking, many published studies still lack sufficient detail to allow another group to reproduce the simulations, let alone to systematically extend or challenge the results. This situation undermines trust in computational results and slows the progress of science in general and the computational field in particular. It also contributes to a broader \emph{reproducibility crisis} documented across many areas of computational science\cite{CH12_toolsRepro2016,CH12_reproWorkflows2017,CH12_barba2022role,CH12_rigorRepro2020}.

The FAIR (\emph{F}indable \emph{A}ccessible \emph{I}nteroperable \emph{R}eusable) Guiding Principles for scientific data management and stewardship were formulated in 2016 to provide high-level, domain-independent guidance for how digital research objects, such as data, metadata, software, and workflows should be published and maintained\cite{CH12_wilkinson2016FAIR,CH12_goFairPrinciples,CH12_nnlmFAIR}. Although FAIR was not written specifically for molecular modelling, its concepts map onto the needs of the computational community\cite{CH12_turingWayFAIR,CH12_healthFAIR2022}. At the same time, efforts have begun to compile practical tips and checklists for performing and reporting meaningful, reproducible molecular docking and simulations\cite{CH12_plos2025tenDocking,CH12_mdChecklist2023,CH12_rigorRepro2020}.
...
For an object to be reusable, it must first be \emph{Findable}. This principle emphasises globally unique and persistent identifiers, such as DOIs, richly described metadata, and indexing in searchable resources\cite{CH12_wilkinson2016FAIR,CH12_goFairPrinciples,CH12_turingWayFAIR}. For computational chemistry, these ideas translate into the following practices:

\begin{itemize}
  \item Assign persistent identifiers, for example, DOIs from Zenodo\cite{CH12_sicilia2017community} or institutional repositories, to curated datasets, input decks, workflows, and software releases.
  \item Provide descriptive metadata that includes, at minimum, the scientific context, such as the targets, ligands, and chemical series. For methods, include the software used, versions, key parameters, and citations to relevant publications.
...
In docking, findability also involves clear linking between the publication, the primary data (e.g., ligand libraries, receptor structures), and the computational workflows used to generate scores and poses\cite{CH12_plos2025tenDocking,CH12_rigorRepro2020}.

\subsection{Accessible}

Accessibility in FAIR refers to the use of open, standardised communication protocols and to the idea that, even if data themselves cannot be openly released, the metadata describing them should remain accessible\cite{CH12_goFairPrinciples,CH12_nnlmFAIR}. From a computational chemistry perspective:
...
Repositories such as Zenodo\cite{CH12_sicilia2017community}, Figshare\cite{CH12_thelwall2016figshare}, Dryad\cite{CH12_miller2016making}, OSF\cite{CH12_foster2017open}, and institutional data infrastructures\cite{CH12_uTexasRepos,CH12_aguRepos,CH12_snsfRepos} provide persistent identifiers and managed access controls that help satisfy the Accessible principle.
...
\begin{itemize}
  ...
  \item When depositing data, provide machine-readable metadata that follows community standards or templates, such as domain-specific checklists or MIxS-like schemas adapted for computational chemistry.
\end{itemize}

Interoperability is critical if one wants to screen docking datasets across targets, benchmark different docking engines, or integrate scores with experimental bioactivity databases such as ChEMBL or BindingDB.

\subsection{Reusable}

The ultimate aim of FAIR is to maximise the potential for reuse of digital objects, whether by the original authors or by others\cite{CH12_wilkinson2016FAIR,CH12_goFairPrinciples,CH12_nnlmFAIR}. Reusability requires rich documentation and explicit licensing conditions. For docking studies, this means:
...
\begin{itemize}
  ...
  \item Definition of the binding site: how the docking box or search region was defined (for example, around a co-crystallised ligand, by pocket detection, or by functional hypotheses), including numerical coordinates and box dimensions.
\end{itemize}
...
\begin{itemize}
  ...
  \item Curation steps: removal of inorganic species and counterions, standardisation of valences and charges, removal of duplicates, rule-based or machine-learning based filters, for example, Lipinski, PAINS, and toxicophores.
...
\begin{itemize}
  ...
  \item Random seeds and parallelisation settings, so that non-deterministic aspects of the search can be reproduced or statistically characterised.
\end{itemize}

Using containers, such as Docker or Singularity, or virtual machines, and making these images available via repositories can dramatically simplify reproducibility by capturing the full software stack\cite{CH12_reproWorkflows2017,CH12_toolsRepro2016}.
...
\begin{itemize}
  ...
  \item Validation steps: redocking of co-crystallised ligands with RMSD thresholds, enrichment metrics on reference actives and decoys, or correlation with experimental binding data.
\end{itemize}
...
In the last decade, a robust ecosystem of generalist data repositories has emerged, many of which support FAIR-aligned data publication at no or low cost. Examples include Zenodo(\textbf{Figure \ref{fig:zenodo}}), Figshare(\textbf{Figure \ref{fig:figshare}}), Dryad, the Open Science Framework (OSF)(\textbf{Figure \ref{fig:osf}}), Mendeley Data, Harvard Dataverse, Vivli, and institutional platforms operated by universities or consortia\cite{CH12_uTexasRepos,CH12_aguRepos,CH12_snsfRepos}. 
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{zenodo.jpg}
    \caption{A screenshot of Zenodo's page} 
    \label{fig:zenodo}
\end{figure}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=1\textwidth]{figshare.jpg}
    \caption{A screenshot of figshare's page} 
    \label{fig:figshare}
\end{figure}
...
To maximise reusability and minimise barriers for reviewers and future users, we recommend organising a docking project for deposition along the following lines:
...
Once organised, this directory tree can be archived and uploaded to a generalist repository such as Zenodo or OSF, which in turn will assign a DOI and allow the dataset to be cited alongside the associated article\cite{CH12_uTexasRepos,CH12_aguRepos,CH12_snsfRepos}.
...
\begin{itemize}
  ...
  \item Archive that releases in a data repository that mints a DOI, for example, Zenodo--GitHub integration, so that the exact code snapshot can be cited and preserved.
...
\section{Checklists for reproducible molecular docking}
...
Building on published recommendations for docking\cite{CH12_plos2025tenDocking} and general computational reproducibility, one can formulate the following checklist.
...
\begin{itemize}
  \item Names, versions, and origin of all key software components, such as dockers, preparation tools, and analysis scripts.
...
\begin{itemize}
  ...
  \item Search parameters, such as exhaustiveness, runs, and time limits.
...
\begin{itemize}
  \item Redocking results for co-crystallised ligands, with RMSD thresholds and statistics.
...
Authors and reviewers can use such a checklist as a concrete implementation of FAIR principles for docking and related computational chemistry workflows\cite{CH12_plos2025tenDocking,CH12_mdChecklist2023,CH12_rigorRepro2020}.

\section{Open versus closed source software and data}

\subsection{Open-source software}

Open-source scientific software, that is, programs released under licences that grant users the rights to run, study, modify, and redistribute the code, has had a transformative impact on many areas of computational research\cite{CH12_tenOpenDev,CH12_tenCleanOSS,CH12_fossCase2021}. Surveys of widely cited bioinformatics tools such as BLAST\cite{CH12_madden2013blast} and ClustalW\cite{CH12_thompson2003multiple} show that open availability of code is often correlated with high scientific impact, in part because others can build upon and validate the methods.
...
A useful concept here is \emph{open verifiable reproducibility}, where the computational workflow and environment are open; however, the sensitive input data are accessed via controlled channels that nonetheless allow independent verification by authorised parties\cite{CH12_computationalReproGuide}.
...
\begin{itemize}
  \item Journals requiring that, when proprietary software is used, authors disclose version details and provide maximal FAIR details such as input files, scripts, and containers.
  ...
  \item Professional societies and training programmes emphasising FAIR and open practices as core competencies for computational chemists.
\end{itemize}

\section{Summary}

For molecular docking and related computational chemistry approaches to attain a level of credibility comparable to that of well-established experimental assays, their results must be reported in a manner that allows independent researchers to reproduce, evaluate, and extend the work. The FAIR Guiding Principles provide a widely recognised framework for organising such transparency. However, their value depends on their translation into concrete, domain-specific practices that can be routinely adopted within computational chemistry. In practical terms, this involves careful documentation of biological targets, ligand libraries, software versions, computational environments, and docking protocols. Equally important is the systematic organisation of workflows and datasets in structured repositories that include persistent identifiers and clearly defined licenses. When feasible, the use of open-source software and openly accessible data further strengthens the transparency and longevity of computational studies. Adopting these practices requires additional effort and gradual adjustments to community standards. Nevertheless, the benefit is a more cumulative and reliable scientific enterprise. In such a framework, docking results are no longer opaque numerical outputs in a table, but reproducible digital experiments whose methods, data, and assumptions are accessible for verification and further development.
