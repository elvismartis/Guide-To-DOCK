...
The DOCK program represents potential binding sites by filling the surface with a set of overlapping spheres, effectively creating a negative image of the receptor’s topography using a program called \verb|sphgen|\cite{CH7_Kuntz1982}. 
\begin{itemize}
...
\begin{enumerate}
  ...
  \item For each generated pose:
        ...
        \end{enumerate}
  \item Store and rank poses based on primary scores. Also, note that there was a secondary score for initial ligand conformations; however, this is now obsolete from DOCK v6.10 onwards.
\end{enumerate}
...
\begin{enumerate}
  \item \emph{Protein preparation}: The input receptor structure is cleaned using Schr\"{o}dinger's Protein Preparation Wizard (PPW). As corrective measures, the PPW performs the following operations on the input receptor structure:
    \begin{itemize}
      ...
      \item Missing backbone atoms, especially from the loop regions, warrants a loop-modelling feature in the prime module.
...
\begin{itemize}
  ...
  \item $\textrm{E}_{\textrm{Lipo}}$: Lipophilic/hydrophobic term derived from a hydrophobic grid potential that rewards favourable burial of nonpolar surface area and lipophilic–lipophilic contacts. This term captures the hydrophobic effect empirically, complementing the van der Waals term.
...
\begin{itemize}
\item \emph{Solis and Wets (SW):} This algorithm is the standard random optimizer used for local search in AutoDock4. It does not require gradient information; instead, it randomly samples the local space and adaptively adjusts its step sizes based on the success or failure of previous moves.
...
\item \emph{Static charges}: Vina assumes a fixed protonation state, and charge distribution that does not change between the bound and unbound states.
\end{itemize}
