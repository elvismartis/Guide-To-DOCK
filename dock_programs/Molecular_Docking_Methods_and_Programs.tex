...
\subsection{The theory between DOCK program}
...
\subsection{Typical DOCK Workflow}
...
The search algorithm is hierarchical and funnel-shaped \cite{CH7_Friesner2004a}. The key stages are:
\begin{enumerate}
  ...
  \item \textbf{Greedy scoring and refinement (Stage 2).} For each retained placement, a series of filters is applied:
    \begin{itemize}
      ...
      \item \emph{Greedy scoring (Step 2c):} All ligand atoms are scored with a discretized empirical potential patterned after ChemScore, rewarding hydrophobic contacts, hydrogen bonds, and metal interactions, while penalizing steric clashes \cite{CH7_Friesner2004a}.
  ...
  \item \textbf{Grid-based energy minimization (Stage 3).} Selected poses are minimized on precomputed OPLS-AA van der Waals and Coulomb grids for the receptor \cite{CH7_Friesner2004a}. For internally generated conformations, this stage includes:
    \begin{itemize}
...
\begin{description}
  ...
  \item[XP (Extra Precision).] A more exhaustive sampling protocol based on anchor-and-grow strategies, coupled with the XP scoring function designed to aggressively penalize poses that violate physical and structural principles (e.g., buried unsatisfied charges, unrealistic desolvation) and to reward well-defined hydrophobic enclosure \cite{CH7_Friesner2006}. XP is slower but offers improved enrichment and better discrimination between actives and decoys.
\end{description}
...
\begin{itemize}
  ...
  \item \textbf{GlideScore:} An empirical scoring function approximating binding free energy, optimized for docking accuracy and enrichment. GlideScore is used to compare different ligands; more negative values indicate more favorable predicted binding \cite{CH7_Friesner2004b}.
\end{itemize}
...
In SP (and related HTVS) docking, the default scoring function is GlideScore, an empirical, ChemScore-inspired function \cite{CH7_Friesner2004a,CH7_Friesner2004b}. In Glide~5.5, the SP GlideScore (\(G_{\text{Score}}\)) is given by:
\[
...
\begin{enumerate}
  ...
  \item[\(E_{\text{Lipo}}\) (Lipo).] Lipophilic/hydrophobic term derived from a hydrophobic grid potential that rewards favorable burial of nonpolar surface area and lipophilic–lipophilic contacts. This term captures the hydrophobic effect in an empirical fashion, complementing the van der Waals term \cite{CH7_Friesner2004b,CH7_Friesner2006}.
  ...
  \item[\(E_{\text{HBond}}\) (HBond).] Hydrogen-bonding term. This is decomposed internally into components for:
    ...
    \end{itemize}
    Each component carries different weights and distance/angle dependence, allowing stronger reward for well-oriented, partially buried hydrogen bonds and lesser reward for solvent-exposed or suboptimal interactions \cite{CH7_Friesner2004a}.
  ...
  \item[\(E_{\text{BuryP}}\) (BuryP).] Penalty for buried polar groups (on either ligand or receptor) that are not engaged in satisfactory hydrogen bonding or ionic interactions. This term helps penalize physically unrealistic poses where charged or strongly polar atoms are desolvated without compensating interactions \cite{CH7_Friesner2004b,CH7_Friesner2006}.

  \item[\(E_{\text{RotB}}\) (RotB).] Penalty for freezing rotatable bonds upon binding. This term approximates the entropic cost of restricting ligand flexibility; more rotatable bonds typically incur a larger penalty unless offset by strong favorable interactions \cite{CH7_Friesner2004b}.
...
\begin{itemize}
  ...
  \item \(E_{\text{desolv}}\), \(E_{\text{buried\_polar}}\): explicit-water-based solvation and buried-polar penalties derived from docking water molecules into the complex and analyzing their interactions with polar/charged groups \cite{CH7_Friesner2006}.
\end{itemize}
