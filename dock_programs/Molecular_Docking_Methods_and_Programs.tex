...
\section{DOCK}
The DOCK program is considered the first tool developed for molecular docking in 1982\cite{CH9_Kuntz1982}. It was developed by the University of California, San Francisco (UCSF) in the group of Irwin Kuntz. It was the first automated procedure to dock small molecules into a receptor site of known structure and remains under active development in the form of DOCK3 and DOCK6 variants\cite{CH9_Meng1992Grid,CH9_Allen2015,CH9_Brozell2012}.

DOCK aims to predict the most favourable binding configurations of small molecules (ligands) within the active sites of macromolecular targets, such as proteins. This computational resource simulates molecular interactions to identify low-energy binding poses, making it an essential asset for drug discovery and structure-based design initiatives. Created to tackle rigid-body docking, DOCK utilises a geometric matching algorithm that aligns the ligand with a negative representation of the receptor's binding pocket. Over the years, its functionality has evolved to incorporate features such as force-field-based scoring, real-time optimisation, and algorithms for flexible ligand docking. The program employs scoring functions to assess and rank potential binding poses according to their predicted binding affinities. It accommodates various input file formats, including PDB, MOL2, and SDF, and is extensively utilised for virtual screening and lead optimisation in drug development. DOCK continues to be a fundamental tool in computational chemistry, renowned for its efficiency in exploring molecular interactions and its ability to adapt to new advancements in docking techniques.

Conceptually, DOCK separates docking into two largely independent
problems:
\begin{enumerate}
  \item \textbf{Sampling}: generating candidate orientations and
        conformations of the ligand in the receptor site.
  \item \textbf{Scoring}: evaluating each candidate pose using
        approximate interaction energies and/or empirical descriptors.
\end{enumerate}
This modular design allows users to combine different sampling schemes
(e.g.\ rigid docking, anchor-and-grow) with different scoring functions
(force-field based, contact-based, pharmacophore-based, etc.).

\section{Fundamental Working Theory of DOCK}

\subsection{Binding-site representation: spheres and shape complementarity}

The distinctive feature of DOCK is its geometric description of the
binding site using overlapping spheres. A separate program, \texttt{sphgen},
fills the binding site with a set of spheres whose centers lie on or
near the solvent-accessible surface of the receptor \cite{CH9__Kuntz1982}. 
The sphere centers are taken as
putative ligand atom positions that capture the shape of the pocket:
\begin{itemize}
  \item Each sphere is defined by a center (three Cartesian coordinates)
        and a radius.
  \item Spheres do not intersect receptor atoms but may overlap each
        other significantly, allowing a relatively small set of spheres
        to approximate cavity shape.
  \item Optional ``coloring'' associates simple chemical types (e.g.\
        hydrogen-bond donors, acceptors, hydrophobes) with spheres to
        enforce basic chemical complementarity during matching.
\end{itemize}

This representation reduces the enormous continuous search space to a
combinatorial matching problem: find sets of ligand atoms that can be
superimposed on subsets of sphere centers while preserving inter-point
distances.

\subsection{Ligand–sphere matching and pose generation}

Given a prepared ligand and a set of sphere centers, DOCK generates
candidate rigid-body orientations as follows
\cite{CH9_Kuntz1982,CH9_Shoichet1992Shape}:

\begin{enumerate}
  \item Choose a small subset (typically 3 or more) of sphere centers
        in the binding site.
  \item Identify subsets of ligand heavy atoms whose inter-atomic
        distances approximately match the distances between the chosen
        sphere centers, within a user-defined tolerance \(\varepsilon\):
        \[
          \left| d_{ij} - d_{IJ} \right| \le \varepsilon,
        \]
        where \(d_{ij}\) is the distance between sphere centers \(i\)
        and \(j\), and \(d_{IJ}\) is the distance between ligand atoms
        \(I\) and \(J\).
  \item For each consistent set of sphere–atom pairs (a \emph{match}),
        compute the rigid-body transformation (rotation and translation)
        that minimizes the root-mean-square deviation (RMSD) between the
        matched ligand atoms and sphere centers.
  \item Apply this transformation to the entire ligand to generate a
        candidate pose.
\end{enumerate}

Various filters are used to reduce the number of matches:
\begin{itemize}
  \item \textbf{Distance tolerance} \(\varepsilon\) on inter-point
        distances.
  \item \textbf{Chemical matching} (sphere coloring) to enforce
        chemically sensible matches.
  \item \textbf{Critical points} (required spheres) to force occupancy
        of key subpockets.
  \item \textbf{Bump checking} to discard poses with severe steric
        clashes before expensive scoring or minimization.
\end{itemize}

In early versions, DOCK primarily considered rigid ligands.
Subsequently, flexible docking strategies such as anchor-and-grow and
genetic algorithms were introduced, which treat the ligand as a set of
rotatable fragments and sample torsional degrees of freedom during or
after initial rigid-body placement\cite{CH9_Oshiro1995,CH9_Meng1993Min}.

\subsection{Grid-based interaction energy evaluation}

Once a pose is generated, DOCK typically evaluates its quality using
precomputed interaction grids. A separate program, \texttt{grid}, computes
the receptor contribution to a molecular mechanics-type potential on a
three-dimensional lattice surrounding the binding site \cite{CH9_Meng1992Grid}. 
The interaction energy between ligand and
receptor is approximated as the sum of van der Waals and electrostatic
terms:
\[
  E_{\text{int}} \approx E_{\text{vdw}} + E_{\text{es}}.
\]

The key idea is to precompute the receptor-dependent part of the
interaction energy at each grid point k and store it in grid files.
During docking, these values are interpolated and combined with
ligand-dependent parameters to yield a fast approximate energy
evaluation for each pose.

This strategy dramatically accelerates docking because receptor
contributions are computed only once, whereas ligand placements are
evaluated many thousands to millions of times.

\subsection{Approximate nature of the scoring function}

In classical DOCK usage, the interaction energy is modeled as a sum of:
\begin{itemize}
  \item Lennard–Jones-type van der Waals interactions
  \item Coulombic electrostatics, often with a distance-dependent
        dielectric
\end{itemize}
and typically does not include explicit hydrogen bonding, solvation, or
conformational entropy terms in the primary scoring function
\cite{CH9_Meng1992Grid}. More sophisticated scoring and
rescoring options (e.g.\ PB/SA, GB/SA, AMBER-based scoring, footprint
similarity) are available in modern DOCK 6 releases
\cite{CH9_Allen2015,CH9_Brozell2012}, but the basic conceptual
framework remains a fast, approximate physical interaction energy.

Because of these approximations, DOCK scores are best interpreted as
\emph{relative} measures for ranking poses or compounds, not as
quantitative binding free energies.

\section{Typical DOCK Workflow}

Although DOCK is highly configurable, a typical DOCK6 workflow for
structure-based virtual screening or pose prediction involves the
following steps\cite{CH9_Allen2015}.

\subsection{Receptor preparation}

\begin{enumerate}
  \item Start from an experimental structure (X-ray, cryo-EM, or NMR)
        of the protein, often a protein–ligand complex.
  \item Clean the structure:
        \begin{itemize}
          \item Remove crystallographic waters unless known to be
  ...
  \item Convert to a format with atom types and partial charges
        (typically MOL2), using external tools (e.g. AMBER, Chimera,
        OpenEye, or other parameterization pipelines).
  \item Define the binding site (e.g. around a bound ligand or
        specified residues) and create a bounding box (via
        \texttt{showbox} or similar utilities).
\end{enumerate}

\subsection{Sphere generation (\texttt{sphgen} and clustering)}

\begin{enumerate}
  \item Use \texttt{sphgen} to fill the binding site with overlapping spheres
        that approximate the shape of the pocket:
        \begin{itemize}
          \item The spheres touch the receptor surface but do not
                intersect receptor atoms.
          \item The density and distribution of spheres can be tuned.
        \end{itemize}
  \item Optionally, cluster and prune spheres using
        \texttt{sphcluster} to:
        \begin{itemize}
          \item Reduce redundancy.
          \item Focus on a particular sub-pocket (e.g. a known
                pharmacophore region).
          \item Designate critical spheres.
        \end{itemize}
  \item Optionally, color spheres with simple chemical labels
        (donor/acceptor/hydrophobe, etc.) for chemical matching.
\end{enumerate}

\subsection{Grid generation (\texttt{grid})}

\begin{enumerate}
  \item Define a three-dimensional grid box that encloses the binding
        site (often oriented to the sphere cluster).
  \item Run \texttt{grid} to precompute:
        \begin{itemize}
          \item \textbf{Bump grid}: encodes severe steric clashes.
          \item \textbf{Contact grid}: for contact-based (shape-like)
                scoring.
          \item \textbf{Energy grid}: for force-field-like scoring
  ...
  \item Choose parameters such as:
        \begin{itemize}
          ...
          \item Cutoff distances for van der Waals and electrostatic
                contributions.
        \end{itemize}
  \item The result is a set of grid files (e.g.\ \texttt{*.nrg},
        \texttt{*.cnt}, \texttt{*.bmp}) that encode receptor potentials.
\end{enumerate}

\subsection{Ligand preparation}

\begin{enumerate}
  \item Generate three-dimensional coordinates for the ligands
        (e.g.\ from SMILES or a database).
  \item Enumerate relevant protonation and tautomeric states.
  \item Assign atom types and partial charges compatible with the
        receptor parameterization.
  \item Identify rotatable bonds and (optionally) pre-generate
        conformer ensembles.
  \item Store ligands in MOL2 (or compatible) format suitable for DOCK.
\end{enumerate}

\subsection{Docking and scoring}

\begin{enumerate}
  \item Run the main docking program (\texttt{dock6}) with a chosen
        sampling strategy:
        \begin{itemize}
          \item Rigid-body docking using sphere–atom matching.
          \item Flexible docking using anchor-and-grow, genetic
  ...
  \item For each generated pose:
        \begin{enumerate}
          ...
          \item Evaluate one or more scoring functions:
                ...
                \end{itemize}
          \item Optionally, perform local minimization (rigid-body or
                torsional) to relax the pose into a nearby local minimum.
        \end{enumerate}
  \item Store and rank poses based on primary and secondary scores.
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Visually inspect top-ranked poses for key interactions,
        steric sanity, and chemical plausibility.
  \item Optionally rescore with more sophisticated methods
        (PB/SA, GB/SA, AMBER score, footprint similarity, etc.).
\end{enumerate}

\section{Scoring Functions in DOCK}

DOCK provides several scoring functions, many of which can be combined
through the descriptor score framework in DOCK6\cite{CH9_Allen2015}. 
For beginners, the most important are:
\begin{itemize}
  \item Force-field-based grid energy score (\textbf{grid score} or
        \textbf{energy score}).
  \item Contact (shape-like) score.
  \item Continuous (Cartesian) energy score.
  \item Footprint similarity and pharmacophore-based scores
        (for more advanced workflows).
\end{itemize}

\subsection{Force-field-based grid energy score}

\subsubsection{Pairwise interaction form}

The conceptual starting point is a pairwise additive interaction
potential between ligand atoms \(i \in L\) and receptor atoms
\(j \in R\):
\[
...
where:
\begin{itemize}
  \item \(r_{ij}\) is the distance between ligand atom i and
        receptor atom j.
  \item \(A_{ij}\) and \(B_{ij}\) are Lennard--Jones parameters
        derived from atomic van der Waals radii and well depths
        (via combining rules).
  \item n and m are the exponents of the generalized Lennard--Jones
        potential (commonly n = 12, m = 6; sometimes softened to
        n = 9, m = 6).
  \item $q_i$ and $q_j$ are partial charges of ligand and receptor
        atoms, respectively.
  \item \(\varepsilon(r_{ij})\) is the effective dielectric, often
        chosen as a distance-dependent form, e.g.
        \(\varepsilon(r) = \varepsilon_0\, r\) with
        \(\varepsilon_0 \approx 4\).
\end{itemize}

In DOCK, these contributions can be scaled:
\[
  E_{\text{grid}} =
    w_{\text{vdw}}\, E_{\text{vdw}} +
    w_{\text{es}}\, E_{\text{es}},
\]
where \(w_{\text{vdw}}\) and \(w_{\text{es}}\) are user-defined scaling
factors.

\subsubsection{Generalized Lennard--Jones form}

For identical atoms with van der Waals radius R and well depth
\(\varepsilon\), DOCK employs a generalized Lennard--Jones expression:
\[
  V_{\text{LJ}}(r) = \varepsilon
    ...
    \right],
\]
with exponents $n > m$ (e.g. n = 12, m = 6 or n = 9, m = 6). 
Constants C and D are chosen such that:

\begin{align*}
  V_{\text{LJ}}(r = R) &= -\varepsilon, \\
  \frac{\mathrm{d} V_{\text{LJ}}}{\mathrm{d}r}\biggr|_{r = R} &= 0,
\end{align*}
which fixes the position and depth of the potential minimum while
allowing the repulsive wall to be softened (e.g.by moving from 12--6
to 9--6).

\subsubsection{Grid-based formulation}

Computing the full double sum over all receptor atoms for each pose is
expensive. DOCK avoids this by precomputing receptor contributions on a grid \cite{CH9_Meng1992Grid}.

For each grid point k, DOCK stores three receptor-dependent
quantities:
\[
  G^{(n)}_k = \sum_{j \in R}
    \frac{A_j}{r_{jk}^{n}},
...
\]
where \(r_{jk}\) is the distance between receptor atom j and grid
point k, and $A_j$ and $B_j$ are atomic-only Lennard--Jones
parameters (before combining with ligand atoms).

Given a ligand pose, each ligand atom i is mapped to the
surrounding grid points (via nearest-neighbor or trilinear
interpolation). Denoting the effective grid index for atom i by
$k(i)$, the interaction energy is approximated as:
\[
  E_{\text{vdw}} \approx
    \sum_{i \in L}
      \left(
...
\[
  E_{\text{es}} \approx
    \sum_{i \in L}
      q_i\, G^{(\text{es})}_{k(i)}.
\]
Here $A_i$ and $B_i$ are ligand atomic parameters, and the
precomputed receptor sums \(G^{(n)}_k\), \(G^{(m)}_k\), and
\(G^{(\text{es})}_k\) are read from the energy grid files.

This formulation transforms an \(O(N_L N_R)\) double sum into an
\(O(N_L)\) sum over ligand atoms, where \(N_L\) and \(N_R\) are the
numbers of ligand and receptor atoms, respectively. Most of the
computational effort is shifted to the one-time grid generation step.

\subsection{Contact (shape-like) scoring}

The \emph{contact score} is a simpler, shape-oriented scoring function
that counts favorable close contacts between ligand and receptor heavy
atoms, while penalizing severe overlaps. 
A contact is defined by a distance cutoff
\(d_{\text{cut}}\), typically around 4.5 \r{A}, and a bump is
defined by an overlap factor $\alpha$ of the sum of van der Waals
radii:

\begin{itemize}
  \item If
        \(\alpha (R_i + R_j) \le r_{ij} \le d_{\text{cut}}\),
        atoms \(i\) and \(j\) are considered to be in contact.
  \item If
        \(r_{ij} < \alpha (R_i + R_j)\),
        the atoms are in severe steric clash (a bump).
\end{itemize}
...
where:
\begin{itemize}
  \item \(s_{\text{contact}} < 0\) denotes a favorable contact
        contribution.
  \item \(s_{\text{bump}} > 0\) is a penalty for steric clash.
\end{itemize}

In practice, DOCK implements this concept using a precomputed contact
grid analogous to the energy grid, which allows efficient evaluation.
The contact score emphasizes steric complementarity and is often used in
combination with other more physically detailed scores.

\subsection{Continuous (Cartesian) energy score}

Instead of relying on grids, DOCK can compute the same non-bonded
interaction energy in continuous Cartesian space. 
This continuous score uses the same
functional form as the grid-based energy:
\[
  E_{\text{cont}} =
    w_{\text{vdw}}\, E_{\text{vdw}} +
    w_{\text{es}}\, E_{\text{es}},
...
but:
\begin{itemize}
  \item Distances $r_{ij}$ are computed directly between ligand and
        receptor atoms (no grids).
  \item Lennard--Jones exponents and dielectric models are adjustable
        (e.g. 12--6 or 9--6; distance-dependent or constant dielectric).
\end{itemize}

Continuous scoring avoids grid discretization artifacts and can be used
as a more accurate secondary score or during local minimization, at the
cost of higher computational expense.

\subsection{Footprint similarity and pharmacophore-based scores (advanced)}

Modern DOCK6 versions provide more elaborate scoring functions, often
used in combination via the descriptor score framework\cite{CH9_Allen2015}:
\begin{itemize}
  \item \textbf{Footprint similarity score}:
        decomposes interactions (van der Waals, electrostatics,
        hydrogen bonding) per receptor residue and compares the
        resulting footprint vectors between a reference and a
        candidate ligand using metrics such as Euclidean distance or
        Pearson correlation.
  \item \textbf{MultiGrid score}:
        computes residue-decomposed interaction energies on multiple
        grids and combines them with footprint similarity.
  \item \textbf{Pharmacophore matching similarity}:
        evaluates the geometric overlap between pharmacophore feature
        sets for a reference and a candidate.
  \item \textbf{Descriptor score}:
        a linear combination of multiple component scores:
        \[
          S_{\text{descriptor}} =
            \sum_k w_k\, S_k,
        \]
        where \(S_k\) may be grid score, continuous score, footprint
        similarity, pharmacophore similarity, or other implemented
        descriptors.
\end{itemize}

These advanced scores are particularly useful when one wants to enforce
similarity to a known active ligand or refine virtual screening hits.



\section{GLIDE}

Glide (Grid-based Ligand Docking with Energetics) is a widely used molecular docking program developed by Schrödinger for predicting protein–ligand binding modes and ranking ligands in virtual screening campaigns \cite{CH9_Friesner2004a,CH9_Friesner2004b}. Glide combines a hierarchical search of ligand poses in a rigid receptor binding site with empirical, grid-based scoring functions derived from force-field and knowledge-based terms. In its standard-precision (SP) and extra-precision (XP) modes, Glide has been shown to give accurate pose prediction and strong enrichment in retrospective screening benchmarks \cite{CH9_Friesner2004a,CH9_Friesner2006,CH9_Friesner2004b}.

\subsection{Fundamental Working Theory}

At a high level, docking in Glide can be viewed as a search-and-score problem: given a receptor structure and a set of ligands, the program searches the conformational, orientational, and positional space of each ligand within a predefined binding site and assigns a numerical score that approximates the binding free energy. Glide assumes:
\begin{itemize}
  ...
  \item Internally flexible ligands, with conformations generated and optimized on-the-fly or supplied externally.
...
The search algorithm is hierarchical and funnel-shaped \cite{CH9_Friesner2004a}. The key stages are:
\begin{enumerate}
  \item \textbf{Site-point search (Stage 1).} The binding site is discretized into ``site points'' on a coarse 2~\AA{} grid covering the active-site region. For each ligand core conformation, distances from site points to the receptor surface are compared to distances from the ligand center to its molecular surface. Only those site points whose local shape is compatible with the ligand are retained as candidate placements.

  \item \textbf{Greedy scoring and refinement (Stage 2).} For each retained placement, a series of filters is applied:
    \begin{itemize}
      \item \emph{Diameter test (Step 2a):} Selected orientations of the ligand diameter (line through the two most distant atoms) are tested, discarding poses with severe steric clashes.
      \item \emph{Subset test (Step 2b):} A subset of polar/metal-binding atoms is scored against the receptor using a discretized, ChemScore-like function.
      \item \emph{Greedy scoring (Step 2c):} All ligand atoms are scored with a discretized empirical potential patterned after ChemScore, rewarding hydrophobic contacts, hydrogen bonds, and metal interactions, while penalizing steric clashes \cite{CH9_Friesner2004a}.
      \item \emph{Refinement (Step 2d):} Top poses from greedy scoring are rigidly refined by small Cartesian translations to improve complementarity.
    \end{itemize}
    Only the best-scoring refined poses (typically on the order of 100–400 per ligand) are passed to the final stages.

  \item \textbf{Grid-based energy minimization (Stage 3).} Selected poses are minimized on precomputed OPLS-AA van der Waals and Coulomb grids for the receptor \cite{CH9_Friesner2004a}. For internally generated conformations, this stage includes:
    \begin{itemize}
      \item Rigid-body translation and rotation of the ligand.
      \item Torsional optimization of core and terminal (rotamer) groups.
    \end{itemize}
    To improve numerical stability, minimization typically starts on smoothed grids and then ``anneals'' onto the full OPLS-AA nonbonded surface.

  \item \textbf{Final scoring (Stage 4).} The minimized poses are evaluated with Glide's proprietary empirical scoring function, GlideScore (for HTVS/SP) or XP GlideScore (for XP mode). Pose selection within a ligand and ranking across ligands use a combination of GlideScore and a model energy, \(E_{\text{model}}\), that places additional weight on the underlying nonbonded interaction energy and (for flexible docking) the internal strain energy of the ligand conformation.
\end{enumerate}
...
\subsection{Practical Workflow for Glide Docking}

In practice, Glide is used within the Schr\"{o}dinger suite with the following workflow:

\subsubsection{Protein and Ligand Preparation}

\begin{enumerate}
  \item \textbf{Protein preparation.} A crystal or cryo-EM structure is prepared (typically with the Protein Preparation Wizard) by:
    \begin{itemize}
      \item Adding hydrogens and assigning bond orders.
      \item Determining protonation states of ionizable groups at the target pH.
      \item Fixing obvious issues (flipped Asn/Gln/His, missing side chains) and resolving steric clashes.
      \item Optionally minimizing the structure with restrained heavy-atom coordinates.
    \end{itemize}

  \item \textbf{Ligand preparation.} Ligands are prepared with LigPrep/Epik or equivalent tools:
    \begin{itemize}
      \item Generation of tautomers and protonation states over a desired pH range.
      \item 3D coordinate generation with low-energy conformations.
...
Glide requires a receptor grid that encodes receptor shape and interaction fields on a 3D lattice. The key steps are:
\begin{itemize}
  \item \textbf{Defining the binding site:} The grid center is typically defined based on a bound ligand, a set of active-site residues, or a region identified by a binding-site detection tool.
  \item \textbf{Box dimensions:} An inner (enclosing) box defines the allowed ligand centroid region, and an outer (grid) box defines the full volume sampled by ligand atoms.
  \item \textbf{Nonbonded interaction fields:} OPLS-AA van der Waals and Coulomb potentials are computed on the grid, often with scaled radii/charges to allow limited receptor ``softness.''
  \item \textbf{Optional constraints:} Hydrogen-bond, metal, and hydrophobic constraints can be encoded in the grid to enforce key pharmacophoric interactions during docking.
\end{itemize}

\subsubsection{Docking Settings and Modes (HTVS, SP, XP)}

Glide offers several precision levels with different trade-offs:
...
\begin{description}
  ...
  \item[SP (Standard Precision).] Default mode for most docking tasks. Uses the full hierarchical search and SP GlideScore, providing a balance between speed and accuracy. Suitable for pose prediction and primary ranking of hits \cite{CH9_Friesner2004a,CH9_Friesner2004b}.
  \item[XP (Extra Precision).] A more exhaustive sampling protocol based on anchor-and-grow strategies, coupled with the XP scoring function designed to aggressively penalize poses that violate physical and structural principles (e.g., buried unsatisfied charges, unrealistic desolvation) and to reward well-defined hydrophobic enclosure \cite{CH9_Friesner2006}. XP is slower but offers improved enrichment and better discrimination between actives and decoys.
\end{description}

In a typical campaign, large libraries are docked first with HTVS or SP, and only the top fraction (e.g., 5--10\,\%) is redocked with XP for more rigorous scoring.

\subsubsection{Pose Selection and Post-processing}

For each ligand, Glide generates multiple poses. Pose selection and ranking use:
\begin{itemize}
  \item \textbf{Emodel:} An internal model energy that combines GlideScore, the nonbonded grid energy (Coulomb + van der Waals), and, for flexible docking, the ligand's internal strain energy. Emodel emphasizes the physical interaction energy and is primarily used to select the best pose of each ligand.
  \item \textbf{GlideScore:} An empirical scoring function approximating binding free energy, optimized for docking accuracy and enrichment. GlideScore is used to compare different ligands; more negative values indicate more favorable predicted binding \cite{CH9_Friesner2004b}.
\end{itemize}
Subsequent post-processing may include visual inspection, clustering of poses, rescoring (e.g., with MM-GBSA), and comparison with experimental SAR.

\subsection{Standard-Precision GlideScore: Functional Form and Terms}

In SP (and related HTVS) docking, the default scoring function is GlideScore, an empirical, ChemScore-inspired function \cite{CH9_Friesner2004a,CH9_Friesner2004b}. In Glide~5.5, the SP GlideScore (\(G_{\text{Score}}\)) is given by:
\[
...
\]
where all terms are in units of kcal\,mol\(^{-1}\). By convention, more negative values of \(G_{\text{Score}}\) correspond to stronger predicted binding.

The components are:
\begin{enumerate}
  \item[\(E_{\text{vdW}}\) (vdW).] Van der Waals interaction energy between ligand and receptor, evaluated on the receptor grid with reduced net ionic charges on groups with formal charges (e.g., metals, carboxylates, guanidinium groups). The coefficient 0.065 scales this term to an empirical binding-energy-like contribution.

  \item[\(E_{\text{Coul}}\) (Coul).] Coulombic (electrostatic) interaction energy between ligand and receptor, also computed on the receptor grid with reduced formal charges. The coefficient 0.130 reflects a somewhat larger relative contribution of electrostatics in the calibrated scoring function.

  \item[\(E_{\text{Lipo}}\) (Lipo).] Lipophilic/hydrophobic term derived from a hydrophobic grid potential that rewards favorable burial of nonpolar surface area and lipophilic–lipophilic contacts. This term captures the hydrophobic effect in an empirical fashion, complementing the van der Waals term \cite{CH9_Friesner2004b,CH9_Friesner2006}.

  \item[\(E_{\text{HBond}}\) (HBond).] Hydrogen-bonding term. This is decomposed internally into components for:
    \begin{itemize}
      \item Neutral–neutral donor–acceptor pairs.
      \item Neutral–charged pairs.
      \item Charged–charged pairs.
    \end{itemize}
    Each component carries different weights and distance/angle dependence, allowing stronger reward for well-oriented, partially buried hydrogen bonds and lesser reward for solvent-exposed or suboptimal interactions \cite{CH9_Friesner2004a}.

  \item[\(E_{\text{Metal}}\) (Metal).] Metal-binding term. Only interactions between metal ions and anionic ligand acceptor atoms are included. If the net metal charge in the apo protein is positive, the scoring function includes a preference for anionic ligands; if the net charge is zero, this preference is suppressed.

  \item[\(E_{\text{BuryP}}\) (BuryP).] Penalty for buried polar groups (on either ligand or receptor) that are not engaged in satisfactory hydrogen bonding or ionic interactions. This term helps penalize physically unrealistic poses where charged or strongly polar atoms are desolvated without compensating interactions \cite{CH9_Friesner2004b,CH9_Friesner2006}.

  \item[\(E_{\text{RotB}}\) (RotB).] Penalty for freezing rotatable bonds upon binding. This term approximates the entropic cost of restricting ligand flexibility; more rotatable bonds typically incur a larger penalty unless offset by strong favorable interactions \cite{CH9_Friesner2004b}.

  \item[\(E_{\text{Site}}\) (Site).] Polar-interaction term in the active site. Polar but non-hydrogen-bonding atoms in a predominantly hydrophobic environment can receive a reward if they are placed in a locally compatible polar microenvironment, or a penalty if they are exposed inappropriately. In practice, this term refines how polar groups are treated in the context of the local binding site.

\end{enumerate}

GlideScore thus blends force-field-based interaction energies (\(E_{\text{vdW}}, E_{\text{Coul}}\)) with empirically calibrated terms capturing hydrophobic enclosure, hydrogen bonding, metal coordination, desolvation penalties, and ligand conformational entropy. The functional form and coefficients are optimized against experimental binding affinities and database enrichment metrics across diverse targets \cite{CH9_Friesner2004b,CH9_Friesner2006}.

\subsection{XP GlideScore: Extended Model of Binding}

The XP (extra-precision) scoring function introduces a more detailed description of protein–ligand recognition and desolvation, especially for hydrophobic enclosure and water-mediated effects \cite{CH9_Friesner2006}. While the full XP functional form is substantially more complex, the top-level decomposition in Glide~4.0 XP can be written as \cite{CH9_Friesner2006}:
\[
...
with:
\begin{itemize}
  \item \(E_{\text{hyd} \cdot \text{enclosure}}\): hydrophobic enclosure term, rewarding ligand groups that are well enclosed by lipophilic protein atoms on opposing faces.
  \item \(E_{\text{HBond} \cdot \text{nn} \cdot \text{motif}}\), \(E_{\text{HBond} \cdot \text{cc} \cdot \text{motif}}\): specialized hydrogen-bond motif terms for neutral–neutral and charged–charged interactions in hydrophobically enclosed environments.
  \item \(E_{\pi\pi}\): \(\pi\)–\(\pi\) and related aromatic stacking interactions.
  \item \(E_{\text{HBond} \cdot \text{pair}}\), \(E_{\text{phobic} \cdot \text{pair}}\): pairwise hydrogen-bond and hydrophobic-contact terms.
  \item \(E_{\text{desolv}}\), \(E_{\text{buried\_polar}}\): explicit-water-based solvation and buried-polar penalties derived from docking water molecules into the complex and analyzing their interactions with polar/charged groups \cite{CH9_Friesner2006}.
\end{itemize}

In XP docking, the sampling algorithm is tightly coupled to XP scoring. Anchor fragments (often rings) from good SP poses are selected, and the ligand is regrown using an anchor-and-grow strategy. During regrowth, XP penalties and rewards guide focused sampling to avoid physically unrealistic poses and to exploit favorable hydrophobic pockets and hydrogen-bond motifs \cite{CH9_Friesner2006}. Because of this coupling, rescoring SP poses with XP GlideScore alone is generally not recommended; XP should be run as a full sampling + scoring protocol.

\subsection{Interpreting Glide Scores in Training and Practice}

For beginners, several practical points are worth emphasizing:
\begin{itemize}
  \item \textbf{Relative, not absolute, binding free energies.} GlideScore and XP GlideScore are empirical approximations to binding free energy, not rigorous thermodynamic quantities. They are most reliable for \emph{relative} ranking of similar ligands docked to the same receptor and grid.

  \item \textbf{Pose selection vs.\ ranking.} Use \(E_{\text{model}}\) (or the XP pose rank property) to select the best pose of each ligand, and GlideScore/XP GlideScore to compare ligands to each other.

  \item \textbf{Mode choice.} HTVS/SP are appropriate for large libraries and initial screening; XP is best reserved for a narrowed set of hits or lead-optimization series.

  \item \textbf{Validation.} For any new target, redock known ligands and compare predicted poses to experimental structures, and check whether GlideScore/XP GlideScore qualitatively correlate with known SAR \cite{CH9_Friesner2004a,CH9_Friesner2004b}.
\end{itemize}


\section{AutoDock}
Autodock or commonly known as AD4, offers an easy-to-set-up and run kind of environment for molecular docking experiments. Therefore, this becomes an easy tool for beginners wanting to learn molecular docking. The developers also separately provide all the tools/scripts, known as "MGLTools", necessary to set up chemical systems for molecular docking. It is, therefore, an automated computational procedure primarily designed to predict the bound conformations of small, flexible ligands to macromolecular targets of known structure. Being an open-source tool with source code available, various groups have expanded the functionality of AD4 to improve docking accuracy for specific and challenging problems.\cite{CH9_Morris1998,CH9_Morris2009} It combines
\begin{itemize}
  \item a semi\-empirical free-energy force field (the scoring function), and
  \item a global optimisation algorithm, most commonly the Lamarckian genetic algorithm (LGA),
\end{itemize}
to search the space of ligand translations, orientations, and internal torsions and to identify low-energy binding poses.

AutoDock4 uses a grid-based strategy for speed: receptor--ligand interaction energies are precomputed on three-dimensional grids (using AutoGrid), and during docking the energies are obtained by trilinear interpolation of these grids rather than explicit pairwise evaluation for each pose.\cite{CH9_Huey2007,CH9_Morris2009} This allows docking of many ligands or extensive conformational sampling of a single ligand at relatively modest computational cost.

\section{Fundamental Working Theory}

\subsection{The docking problem as a free-energy optimisation}
...
\subsection{Representation of receptor and ligand}

In standard AutoDock 4:
\begin{itemize}
  \item The receptor is treated as rigid, although limited side-chain flexibility can be enabled by defining selected receptor residues as flexible (these flexible parts are technically treated as part of the ligand subsystem).\cite{CH9_Morris2009}
...
Atomic partial charges are typically Gasteiger charges, and atom types are assigned according to AutoDock's built-in atom typing scheme, which is essential for both the interaction grids and the desolvation model.\cite{CH9_Huey2007}

\subsection{Search algorithms: genetic algorithms and the Lamarckian GA}

AutoDock historically offered several global search methods (simulated annealing, genetic algorithms, and the Lamarckian genetic algorithm); for AutoDock 4, the default and recommended method is the Lamarckian genetic algorithm (LGA).\cite{CH9_Morris1998,CH9_Morris2009}
...
\begin{itemize}
  ...
  \item A local search (e.g. Solis--Wets or a gradient-based method) is periodically applied to individual candidates to descend into nearby local minima of the scoring function.\cite{CH9_Morris1998,CH9_SantosMartins2021}
...
In practice, a docking job consists of many independent LGA runs (e.g. 50), each starting from different random initial populations. The resulting best poses are then clustered by RMSD, and clusters ranked primarily by cluster size and cluster representative energy.\cite{CH9_Morris2009}

\section{The AutoDock4 Scoring Function}

\subsection{Semi-empirical free-energy force field}

AutoDock 4 uses a semi-empirical free-energy force field that estimates the binding free energy as a sum of physically motivated terms: van der Waals dispersion/repulsion, directional hydrogen bonding, electrostatics with a distance-dependent dielectric, a charge-based desolvation term, and an entropic penalty for ligand torsions.\cite{CH9_Huey2007,CH9_SantosMartins2021}
...
\subsection{Interpretation of individual terms}

\subsubsection{Van der Waals dispersion/repulsion}
...
\[
  ...
  \right),
\]
is a modified Lennard--Jones 12--6 potential. The coefficients $ A_{ij}$ and $B_{ij}$ depend on the atom types of i and j; they are chosen so that the minimum of the potential corresponds to the equilibrium contact distance and well depth appropriate to those atom types.\cite{CH9_Huey2007}
...
\subsubsection{Directional hydrogen bonding}
...
\[
  ...
  \right).
\]
Here $ C_{ij} $ and $ D_{ij}$ define a 12--10 potential specific to hydrogen bond donor--acceptor pairs, providing a narrower, more directional well than the generic van der Waals term.\cite{CH9_Huey2007} The angular term $E(t_{ij})$ depends on the hydrogen bond geometry (angle $ t_{ij} $); it down weights poorly aligned donor--acceptor pairs and favours near linear geometries.
...
\[
  ...
  \right).
\]
The parameters $ S_i $ are solvation descriptors that depend on atom type and, via a charge\-based parametrisation, on the atomic charge.\cite{CH9_Huey2007} Positive $S_i$ correspond roughly to hydrophobic atoms (favourable desolvation on burial), whereas negative values correspond to polar/charged atoms (unfavourable desolvation).
...
\subsubsection{Torsional entropy penalty}

Finally, the torsional entropy term
\[
  \Delta G_{\text{tor}} = W_{\text{tor}} \, N_{\text{tor}}
\]
penalises ligands with many rotatable bonds. When a ligand binds, many internal torsions become restricted, resulting in a loss of conformational entropy. AutoDock uses a simple linear model in the number of rotatable bonds $ N_{\text{tor}}$, with $W_{\text{tor}}$ obtained from regression against binding data.\cite{CH9_Huey2007}

\subsection{Parameterisation and calibration}

The functional form of Eq.~\eqref{eq:ad4_scoring} was chosen based on physical intuition and computational efficiency, and the associated parameters $A_{ij}$, $B_{ij}$, $C_{ij}$, $D_{ij}$, $S_i$, $V_i$, $\sigma$ and weights $W_{\cdot}$ were fitted using linear regression to a dataset of protein--ligand complexes with known experimental binding free energies.\cite{CH9_Huey2007,CH9_Morris2009} The AutoDock 4 force field is thus semi-empirical: grounded in physical models but tuned to reproduce experimental data as closely as possible.

\section{AutoDock 4 Workflow}
...
AutoDock accelerates docking by precomputing three\-dimensional interaction energy maps for each ligand atom type using AutoGrid.\cite{CH9_Morris2009}
...
\begin{enumerate}
  ...
  \end{itemize}
  \item Use the predicted $\Delta G_{\text{binding}}$ values primarily for relative ranking of ligands rather than absolute binding affinity prediction; AutoDock's semi-empirical scoring tends to perform better for ranking within a series than for absolute affinities.\cite{CH9_Huey2007,CH9_Morris2009}
\end{enumerate}
\end{enumerate}
...
\section{Introduction to AutoDock Vina}

AutoDock Vina (usually referred to as Vina) is an open-source molecular docking program developed at The Scripps Research Institute to predict the binding mode and an approximate binding affinity of small molecules (ligands) to macromolecular receptors, typically proteins \cite{trott2010vina}.[cite:7] 
Compared with its predecessor AutoDock 4, Vina introduces a new empirical scoring function, a continuous and differentiable potential tailored for gradient-based optimization, and efficient global search with multithreading support, leading to substantially improved speed and pose prediction accuracy \cite{trott2010vina,forli2016autodock}.[cite:7][cite:11] 
...
From a theoretical standpoint, docking can be seen as a compromise between physically detailed but expensive methods (e.g., molecular dynamics with explicit solvent) and fast but approximate approaches \cite{trott2010vina}.
Instead of explicitly simulating solvent and full receptor flexibility, Vina treats the receptor as largely rigid, the ligand as flexible around specified rotatable bonds, and uses a scoring function trained on experimental protein–ligand complexes (PDBbind) to approximate the standard chemical potentials (binding free energies) of complexes \cite{trott2010vina}.

\section{Fundamental Working Theory}

\subsection{Conceptual view of molecular docking}
...
Exact evaluation of \( G(\mathbf{x}) \) requires integrating over solvent and many internal degrees of freedom, which is computationally prohibitive for routine screening \cite{sousa2006docking}.
...
Vina follows this general framework but emphasizes (i) a smooth, differentiable scoring function that supports gradient-based local optimization; (ii) a compact set of empirically fitted parameters; and (iii) efficient use of multi-core CPUs via multithreading \cite{trott2010vina}.

\subsection{Representation of the docking problem in Vina}
...
\begin{enumerate}
  ...
  \item Uses a global search algorithm (iterated local search) combined with a local optimizer (BFGS quasi-Newton) to drive the system towards low-scoring (low “energy”) conformations \cite{trott2010vina}.
\end{enumerate}
...
\section{AutoDock Vina Scoring Function}
...
The conformation-dependent part of the scoring function is defined as \cite{trott2010vina}:
\[
...
The final predicted binding affinity is then derived from the intermolecular component of the lowest-scoring pose and a simple entropic term penalizing ligand flexibility \cite{trott2010vina}.[cite:7]

\subsection{Surface distance and interaction functions}

Instead of expressing interactions directly as functions of \( r_{ij} \), Vina uses the \emph{surface distance} between atoms \cite{trott2010vina}:
\[
...
All interaction functions are truncated beyond a cutoff distance (currently \( r_{ij} = 8 \,\text{\AA} \)) to limit computational cost \cite{trott2010vina}.

\subsection{Elementary interaction terms}

Vina defines the following steric terms as functions of \( d \) (in \AA) \cite{trott2010vina}:
\[
...
\subsection{Weights and final scoring expression}

The empirical scoring function combines these terms linearly. 
In the original Vina parameterization, the weights are \cite{trott2010vina}:
\[
...
\subsection{Predicted binding affinity}

Vina converts the conformation-dependent score into a predicted binding affinity via a simple function \( g \) acting on the intermolecular part of the best-scoring pose and an entropic penalty for ligand flexibility \cite{trott2010vina}:
\[
...
It is important to emphasize that this is an \emph{empirical} estimate: absolute values are approximate, and the scoring function is primarily optimized for pose prediction and ranking, not for quantitative free-energy prediction \cite{trott2010vina,forli2016autodock}.[cite:7][cite:11]
...
\section{Optimization Algorithm and Search Strategy}
...
\begin{enumerate}
  ...
  \item Acceptance or rejection according to a Metropolis-like criterion, allowing occasional uphill moves to escape local minima \cite{trott2010vina}.
\end{enumerate}

Multiple runs are performed from random starting conformations, and each run yields one or more low-scoring local minima. 
Vina then clusters these poses based on RMSD (typically using a cutoff such as 2 \AA) and reports cluster representatives ranked by predicted affinity.
...
Multithreading allows several independent search trajectories to run concurrently on different CPU cores \cite{trott2010vina}.

\section{Practical Workflow for Beginners}

The following workflow summarizes how a typical user would perform a docking experiment with Vina, using either AutoDockTools (ADT)/ADFR tools or the newer Meeko-based scripts \cite{forli2016autodock,butt2020chimera,vinadocs}.[cite:11][cite:8]

\subsection{Step 1: Receptor preparation}
...
\begin{enumerate}
  ...
  \item Assign atomic partial charges, e.g., Gasteiger charges (ADT/ADFR) or charges via Meeko-based workflows \cite{forli2016autodock,vinadocs}.
...
\subsection{Step 2: Ligand preparation}
...
\subsection{Step 3: Defining the search space (grid box)}
...
num_modes      = 9
energy_range   = 3
\end{verbatim}[cite:11]

\subsection{Step 4: Running AutoDock Vina}
...
\begin{itemize}
  ...
  \item \verb|--exhaustiveness|: controls the thoroughness of the global search; default is 8, larger values such as 16–32 improve robustness at higher cost \cite{forli2016autodock,vinadocs}.
  \item \verb|--num_modes|: maximum number of output modes (poses) to report.
  \item \verb|--energy_range|: maximum energy difference (in kcal/mol) between the best and worst reported poses.
\end{itemize}
...
\subsection{Step 5: Analyzing docking results}
...
\begin{enumerate}
  ...
  \end{itemize}
  \item For training and teaching, stepwise protocols such as those implemented in UCSF Chimera plus Vina provide an accessible workflow for beginners \cite{butt2020chimera}.[cite:8]
\end{enumerate}
...
\begin{itemize}
  \item The predicted binding energies are \emph{relative} and approximate; they are most useful for ranking ligands against the same target, not as absolute free energies.
  ...
  \item For critical projects, docking should be complemented by further refinement (e.g., MD simulations, MM/PBSA or MM/GBSA rescoring, or more rigorous free-energy calculations) \cite{forli2016autodock}.
\end{itemize}
...
The Surflex-Dock scoring function is formulated in terms of atomic surface distances between protein and ligand atoms.\cite{CH9_Jain2007Surflex21}
