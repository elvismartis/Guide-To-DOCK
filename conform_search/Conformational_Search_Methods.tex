...
Simulated annealing (SA) is a powerful stochastic optimization technique widely used in conformational search for molecular docking, where the goal is to locate low‑energy ligand and protein conformations compatible with experimental binding modes. In SA, the system is first “heated” to a high virtual temperature, allowing large, energetically unfavorable moves that overcome local minima, and then gradually cooled so that the trajectory converges toward deep basins of the potential energy landscape. This mapping of thermodynamic annealing onto conformational space makes SA particularly suited to global‑optimization problems such as flexible‑ligand docking, where the search space is rugged and high‑dimensional.
