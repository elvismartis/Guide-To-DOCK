...
\include{Chapter 2: Conformational Search Methods}
...
\include{Chapter 3: Scoring Functions}
...
Where $w(r)$ is the final score which is dependent on the atomic distances $r$. $k_{B}$ is the Boltzmann constant, $T$ is the absolute temperature in $K$. $\rho(r)$ is the frequency of protein-ligand atom pairs at distance $r$, and $\rho^*(r)$ is the frequency of protein-ligand atom pairs in the reference state at distance $r$. {\color{red}{The reference state is generally the frequency that a particular atom pair is observed in the experimental structures.}} The type of interaction (intermolecular atom pairs) that appears more frequently in the reference state will be scored higher. Therefore, the ratio $\frac{\rho(r)}{\rho^*(r)}$ can be considered as a pair distribution function.
...
\include{Chapter 4: Know your biological target and its functional state}
...
\include{Chapter 5: Choose the receptor entry in the {\color{red}{wwPDB}} that is closer to the biological problem at hand}
{
...
\include{Chapter 6 : {\color{red}{Druggability is the key when defining a potential binding site for docking}}}
...
\include{Chapter 7: Learn the fundamentals of the docking algorithm}
...
\include{Chapter 8: Filter and prepare ligands to enhance accurancy and speed}
...
\include{Chapter 11: Be consistent with the atomic representations and figures}
...
\include{Chapter 12: Communicate the results to the experimentalists}
...
\include{Chapter 13: Share input files and docking parameters to comply with FAIR guidelines}
...
\begin{table}[ht]
    \centering 
    \caption{{\color{red}{List of repositories for share files in the public domain}}}
    \label{tab:list3}
    \begin{tabular}{|c|c|}
    \hline
...
\include{Important terminologies}
...
\include{Chapter 14: Summary and Take Home Message}
