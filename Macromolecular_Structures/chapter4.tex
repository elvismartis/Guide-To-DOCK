...
\begin{figure}[hbt!]
    ...
    \caption{The RCSB Protein Data Bank (PDB) homepage highlighting navigation options to deposit, search, visualise, and analyse experimentally determined and computed 3D structures} 
...
The output from X-ray crystallography relevant to docking is the atomic coordinates, deposited in PDB or mmCIF format for protein, nucleic acids, ligands, cofactors, solvent, and ions. It also gives us structure factors in MTZ or mmCIF format, which contain diffraction amplitudes and, in some cases, phases or anomalous differences. The metadata describe the experimental conditions and global quality metrics such as resolution, R-factors, and completeness
...
Several crystallography-specific factors affect the reliability of the docking structure. Crystal packing can lead to stabilisation of loop conformations or terminal regions that are weakly populated or absent in solution\cite{CH4_Joosten2009_PDB_related_databanks,CH4_Krissinel2007_PISA}.  Most crystals are measured at cryogenic temperatures to avoid structural damage due to electron bombardment.  This can lead to freezing, which is likely to induce artefacts that can slightly distort structures or redistribute conformational populations. Flexible loops and ligands may be modelled with high $\beta$-factors, partial occupancy, alternate conformations, or omitted altogether. Crystallisation buffers, salts, precipitants, and additives, such as polyethene glycols, organic solvents, can bind to the protein and stabilise non-native conformations or oligomeric states. Starting models for molecular replacement and strong refinement restraints can bias the electron density map towards an incorrect conformation when the data are ambiguous.\cite{CH4_karplus2012linking,CH4_Emsley2010_Coot}


\subsection{Single Particle Cryo-Electron Microscopy}

Single particle cryo-EM reconstructs 3D maps(\textbf{Figure \ref{fig:cryoEMmap}}) of the Coulomb potential from 2D projection images of vitrified biomolecular particles. Individual particle images are aligned and classified, and a 3D reconstruction is computed. Models are then fitted and refined into these maps. The Electron Microscopy Data Bank (EMDB)\cite{CH4_wwpdb2024emdb} is the global archive of 3D CryoEM maps, which are often linked to atomic models deposited in the PDB. An entry in the EMDB that can be downloaded contains the atomic coordinates in PDB/mmCIF format. The cryo-EM maps and the associated metadata can be imported in the MRC/CCP4 format. And lastly, the validation reports, including global and local resolution estimates, and the map--model fit metric, can also be obtained.\cite{CH4_pintilie2021validation,CH4_Iudin2016_EM_validation}
...
\subsubsection{Caveats Relevant to Docking}
The global resolution can be misleading, so the nominal overall resolution may not reflect the local resolution at the binding site. This can lead to poorly resolved side chains and ligands, even at comparatively high nominal resolutions. In some cases, multiple conformational states may be averaged, leading to smeared density and ambiguous side-chain or loop positions. The sharpening procedures intended to enhance high-resolution images can generate artefactual density features or exaggerate noise, potentially leading to overfitting of ligands or side chains. The use of tight masks in refinement can inflate FSC estimates and obscure the true extent of model overfitting. As with X-ray crystallography, proton positions and protonation states are not directly resolved.
...
\begin{figure}[hbt!]
    ...
    \caption{2D \textsuperscript{15}N-HSQC spectrum of NleG2-3[90–191] at \ang{25}C and pH 7.0. Selected peaks corresponding to reduced and oxidised conformations are labelled as r (green) and o (blue), respectively. The image is reproduced from \url{https://commons.wikimedia.org/wiki/File:HSQC-NMR.tiff} under the Creative Commons Attribution 2.5 Generic license.} 
\label{fig:NMR}
\end{figure}

Solution NMR spectroscopy determines structural information from nuclear spin interactions (see \textbf{Figure \ref{fig:NMR}}), such as nuclear Overhauser effects (NOEs), scalar couplings, residual dipolar couplings, and chemical shifts. These are translated into distance and angle restraints, from which atomic models are calculated, often yielding an ensemble of conformers consistent with the data\cite{CH4_Ulrich2008_BMRB,CH4_baskaran2024restraint}.  The associated data are archived in the PDB and in the Biological Magnetic Resonance Data Bank (BMRB)\cite{CH4_Ulrich2008_BMRB}, and comprise the following output:
\begin{itemize}
...
Global analyses of NMR structures reveal that the secondary structure elements are usually well defined; however, the overall structures may be too flexible, particularly in loops and termini, reflecting under-restraint rather than true intrinsic disorder\cite{CH4_fowler2020method,CH4_baskaran2024restraint}. The ensemble RMSD and restraint counts are rough indicators. Whereas newer methods, such as ANSURR, compare predicted rigidity from the structure to that inferred from chemical shifts, providing more nuanced accuracy estimates \cite{CH4_fowler2020method}. The geometry metrics, such as, the Ramachandran outliers, rotamers, and clashes correlate with better agreement to independent data.\cite{CH4_Williams2018_MolProbity,CH4_baskaran2024restraint}

\subsubsection{Caveats Relevant to Docking}
The NMR structures are particularly valuable for capturing conformational dynamics and alternative states. However, docking workflows must select conformers carefully and, when possible, cross-validate against crystallographic or cryo-EM models. From a docking perspective, NMR structures have characteristics that require special care. Different conformers obtained as ensembles can be equally consistent with experimental restraints; therefore, choosing a single arbitrary conformer may bias docking. The Loops or side chains may be weakly restrained, which are effectively modelled rather than determined. Therefore, these regions may not provide reliable structural information; moreover, if they occur within the binding site, the pocket geometries may not be fully correct. Since NMR measurements do not yield density information, there are no electron density or EM maps to inspect. Therefore, the quality must be judged by the completeness of the restraints, the presence of violations, and the comparison with independent data.  While NMR can determine oligomeric complexes, the stoichiometry and symmetry can be underdetermined in some cases.
...
In the context of molecular docking, structural information can help assess how well the experimental data constrain an NMR ensemble. It can provide valuable input for integrating chemical shift-based information on local structure and dynamics with crystallographic or cryo-EM models.

\subsection{EMDB}
The EMDB\cite{CH4_wwpdb2024emdb} provides curated access to three-dimensional EM maps, with associated metadata and validation information. Many PDB entries derived from cryo-EM experiments specify an associated EMDB map identifier, making it straightforward to access the underlying map and validation report. It serves to provide processed maps and half-maps to assist in an independent validation. The global and local resolution estimates are helpful for evaluating the overall structure and, more importantly, the quality of the binding site. It also provides cross-links to fitted atomic models and related experimental data\cite{CH4_pintilie2021validation}. Therefore, docking users can directly assess whether a particular binding site is supported by strong local density, and whether the fitted ligand or side-chain conformations are justified.
...
A structure with resolution better than about 2.5 \r{A}, complete data, and reasonable R-factors is generally suitable as a starting point, though local issues may still be present\cite{CH4_Wang2015_structure_quality,CH4_shao2017multivariate}. Extremely large $\Delta R$ values ($>$0.07) or very poor R-factors at the nominal resolution suggest either problematic data or over- or under-fitted models.
...
\begin{itemize}
  \item Ignore remote, non-critical segments: Terminal regions far from the binding site can often be neglected, unless one wishes to perform MD simulations after docking.
...
\begin{itemize}
\item \textbf{Asymmetric unit vs biological assembly}: PDB entries specify the \emph{asymmetric unit} (ASU), the minimal crystallographic repeating unit. Furthermore, the transformation matrices (BIOMT/BIOMATRIX) define the putative biological assemblies.

\item \textbf{Use PDBePISA to analyse interfaces}: The PDBePISA\cite{CH4_Krissinel2007_PISA} evaluates crystal packing interfaces and predicts likely biological oligomers. It reports the buried surface area and solvation free-energy gain ($\Delta \textrm{G}$) upon complex formation. It shows the hydrogen bonds, salt bridges, and hydrophobic contacts. It also provides a probability score for each interface, indicating whether it is biologically significant. Interfaces with large buried surface area, favourable $\Delta \textrm{G}$, and hydrophobic cores are more likely to be physiologically relevant. Whereas small, polar contacts are more likely to be crystal artefacts.
...
For docking at interfaces, it is always recommended to construct the predicted biological assembly, rather than using the ASU by default.
...
\item \textbf{Interpretation within a single structure}: Within one structure,  the relative B-factors can help identify rigid cores (low B-factors) and flexible loops or termini (high B-factors). However, very high B-factors or partial occupancy on ligands and side chains suggest poorly defined positions. The B-factors are influenced by refinement choices, such as individual vs group refinement, TLS models, and resolution, and should not be interpreted as quantitative measures of physical mobility.\cite{CH4_Wang2015_structure_quality,CH4_shao2017multivariate}
...
Selection of the receptor structure should take the following limitations into account:
\begin{itemize}
...
\begin{itemize}
  \item A user searches for a protein of interest using a PDB portal (e.g., RCSB PDB or PDBe) by name or UniProt accession.
